...
&ensp;&ensp;&ensp;&ensp;采用随机的不同颜色填充地图的整个区域 针对区要素，强调数据的地理位置差异。

### 示例实现

&ensp;&ensp;&ensp;&ensp;本示例需要使用【include-mapboxgl-local.js】开发库实现，通过关键接口`Zondy.Service.ThemeOper()`实现 MapGIS 地图文档的`随机专题图`操作功能。
...
&ensp;&ensp;&ensp;&ensp;创建**专题图结构信息对象**，并将该对象存放数组中，执行专题图功能时使用，如`添加专题图`、`删除专题图`。重点内容：使用`Zondy.Object.Theme.CRandomTheme()`实现**随机**专题图；
