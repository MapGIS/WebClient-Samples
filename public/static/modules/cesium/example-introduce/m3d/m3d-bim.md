...
&ensp;&ensp;&ensp;&ensp;本示例实现在三维场景中加载 M3D 的 BIM 模型数据，对接 MapGIS IGServer 发布的三维地图服务。
...
&ensp;&ensp;&ensp;&ensp;数据准备：本示例采用的数据经过两个步骤生成，首先需在 MapGIS Desktop 桌面平台软件中为 BIM 模型数据生成 M3D 缓存，并组织为地图文档；然后在 MapGIS Server Manager 服务管理器中根据地图文档发布为三维地图服务。
