  ...
  <div :class="{ 'mapgis-header': true, 'mapgis-header-mobile': mobile }">
    ...
    <el-menu
      ...
      <template v-for="(itemF, iF) in headers">
        <el-menu-item :index="`${itemF.title}`" :key="`menu-${iF}`" class="el-menu-item-header" @click="returnLastActive()" v-if="typeof(itemF.children) !== 'undefined' && itemF.children.length > 0">
          <el-submenu :index="`${itemF.title}`" :key="`submenu-${iF}`" :show-timeout="timeout" :hide-timeout="timeout" popper-class="el-submenu-header" :popper-append-to-body="isToBody">
            <template slot="title">{{ itemF.title }}</template>
            <template v-for="(itemS, iS) in itemF.children">
              <div :key="`group-title-${iS}`" v-if="itemS.group" style="padding: 4px 0 4px 10px;font-weight: 600;height:24px">
                {{itemS.group}}
              ...
              <el-submenu :index="`${itemF.title}-${itemS.title}`" :key="`group-item-${iS}`" v-if="typeof(itemS.children) !== 'undefined' && itemS.children.length > 0" :show-timeout="timeout" :hide-timeout="timeout" popper-class="el-submenu-header" :popper-append-to-body="isToBody">
                <template slot="title">{{ itemS.title }}</template>
                <template v-for="(itemT, iT) in itemS.children">
                  <el-menu-item :index="`标识${iF}-${iS}-${iT}`" :key="`group-item-menu1-${iT}`" class="el-menu-item-header" @click="returnLastActive()" v-if="isLink(itemT.url) || isDocs(itemT.url) || isVue3Rsource(itemT.url)">
                    <div>
                      <a :href="itemT.url" target="_blank">{{ itemT.title }}</a>
                    </div>
                  </el-menu-item>
                  <el-menu-item :index="`${iF}-${iS}-${iT}`" :key="`group-item-menu2-${iT}`" class="el-menu-item-header" v-else>
                    <router-link :to="itemT.url">{{ itemT.title }}</router-link>
                  </el-menu-item>
                </template>
              </el-submenu>
              <el-menu-item :index="`标识${itemF.title}-${itemS.title}`" :key="iS" class="el-menu-item-header" @click="returnLastActive()" v-else-if="isLink(itemS.url) || isDocs(itemS.url) || isVue3Rsource(itemS.url)">
                <div>
